...
int startValuePwmMotor = 50;             // начальное значение мощности моторов в П-регуляторе в %
...
//сигмоида
float modExp(float value) {
  return (exp(value / 9.0));
}

// робот "Линия с 1 датчиком - пропорциаональный регулятор"
void line1RobotProp() {
  lastActionMillis = millis();
  ...
  int err = sampleFiltered - lineSensorThresholdValue;
  int leftM = constrain(int(50 + koefProp * err), 0, 100);
  int rightM = constrain(int(50 - koefProp * err), 0, 100);
  ...
  int curLM = map(leftM, 0, 100, 0, maxPwm);
  int curRM = map(rightM, 0, 100, 0, maxPwm);
  if (curLM <= 40) {
    int modLeftM = constrain(modExp(curLM), 0, 255);
    analogWrite(leftMotorPwmPin, modLeftM);
  } else {
    analogWrite(leftMotorPwmPin, curLM);
  }
  if (curRM <= 40) {
    int modRightM = constrain(modExp(curRM), 0, 255);
    analogWrite(rightMotorPwmPin, modRightM);
  } else {
    analogWrite(rightMotorPwmPin, curRM);
  }
