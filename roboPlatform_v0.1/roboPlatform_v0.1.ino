...
unsigned int butReadTime = 25;           // период опроса кнопки в миллисекундах
unsigned long startButPressTimer = 0;    // стартовое значение таймера удержания
unsigned long butPressTimer = 0;         // таймер удержания
unsigned int lastButTime = 0;            // предыдущее значение проверки состояния кнопки
bool butIsUp = true;                     // флаг отпускания кнопки
bool butIsDown = false;                  // флаг нажатия кнопки
...
byte setMotorSpeed = 50;                 // переменная для настройки мощности мотора 0-100%
byte setForwardSec = 5;                  // переменная для настройка времени движения робота по прямой
unsigned int startForwardRobot = 0;      // время старта робота "Движение по прямой"
...
void loop() {
  while (1) {
    ...
    //------------------ заставка -----------------------------------
    //----------------- опрос кнопки --------------------------------
    if (millis() - lastButTime >= butReadTime) {
      butCentreRead();
      lastButTime = millis();
    }
    //----------------- опрос кнопки --------------------------------
...
void butCentreRead() {
  ...
  // кнопка нажата, до этого не нажата = только нажали = "down"
  if (butState and butIsUp) {
    butIsUp = !butIsUp;
    butIsDown = !butIsDown;
  }

  // кнопка нажата и была нажата = удерживаем = "count time"
  if (butState and butIsDown) {
    butPressTimer = millis() - startButPressTimer;
    if (butPressTimer >= butCentreHoldTime) {
      lcd.setCursor(0, 0);
      lcd.printByte(0b01111111);                    // если произошло длительное нажатие, то рисуем обратную стрелку "<-"
    }
    butTimerIsCount = true;
  }
  ...
  // кнопка не нажата, но была нажата = отпустили = "up"
  if (!butState and butIsDown) {
    butIsUp = true;
    butIsDown = false;
    butTimerIsCount = false;
